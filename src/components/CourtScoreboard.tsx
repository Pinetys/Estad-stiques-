...
export const CourtScoreboard: React.FC<CourtScoreboardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Rival Quick Score Bar (Thumb-Friendly, Compact) */}
      <div className={`max-w-xl mx-auto ${compact ? 'mt-0.5 pt-0.5' : 'mt-1 pt-1'} border-t border-neutral-800 flex items-center justify-between gap-1 ${isActionsLocked ? 'opacity-40 pointer-events-none' : ''}`}>
        <span className="text-sky-400 font-mono font-black text-[9px] sm:text-[11px] uppercase tracking-wider shrink-0">
          Rival:
        </span>
        <div className="flex items-center gap-1 grow justify-end flex-wrap">
          <button
            type="button"
            onClick={() => onLogOpponentAction('OPP_1P')}
            disabled={isActionsLocked}
            className={`${compact ? 'h-6 px-1.5 text-[10px]' : 'h-7 sm:h-8 px-2 sm:px-2.5 text-[11px] sm:text-xs'} bg-sky-950/90 hover:bg-sky-900 active:bg-sky-800 text-sky-100 border border-sky-600/70 rounded font-black font-mono transition active:scale-95 shadow-sm disabled:opacity-40 flex items-center justify-center`}
          ...
          <button
            ...
            disabled={isActionsLocked}
            className={`${compact ? 'h-6 px-1.5 text-[10px]' : 'h-7 sm:h-8 px-2 sm:px-2.5 text-[11px] sm:text-xs'} bg-sky-900 hover:bg-sky-800 active:bg-sky-700 text-white border border-sky-400/80 rounded font-black font-mono transition active:scale-95 shadow-sm disabled:opacity-40 flex items-center justify-center`}
          ...
          <button
            ...
            disabled={isActionsLocked}
            className={`${compact ? 'h-6 px-1.5 text-[10px]' : 'h-7 sm:h-8 px-2 sm:px-2.5 text-[11px] sm:text-xs'} bg-blue-900 hover:bg-blue-800 active:bg-blue-700 text-white border border-blue-400/80 rounded font-black font-mono transition active:scale-95 shadow-sm disabled:opacity-40 flex items-center justify-center`}
          ...
          <button
            ...
            disabled={isActionsLocked}
            className={`${compact ? 'h-6 px-1.5 text-[10px]' : 'h-7 sm:h-8 px-2 sm:px-2.5 text-[11px] sm:text-xs'} bg-rose-950/90 hover:bg-rose-900 active:bg-rose-800 text-rose-100 border border-rose-500/80 rounded font-black font-mono transition active:scale-95 shadow-sm disabled:opacity-40 flex items-center justify-center`}
          ...
          <button
            ...
            disabled={isActionsLocked}
            className={`${compact ? 'h-6 px-1.5 text-[9px]' : 'h-7 sm:h-8 px-2 sm:px-2.5 text-[10px] sm:text-[11px]'} bg-neutral-900 hover:bg-neutral-800 text-neutral-300 border border-neutral-700 rounded font-mono font-bold disabled:opacity-40 flex items-center justify-center`}
